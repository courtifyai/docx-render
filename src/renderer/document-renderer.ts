...
type InternalRendererOptions = Required<
  Omit<IRendererOptions, "onCommentAccept" | "onCommentReject">
> &
...
export class DocumentRenderer {
  ...
  private renderSinglePage(
    ...
    const documentArea = this.createElement(
      "div",
  ...
  private renderWithPages(wrapper: HTMLElement, document: IDocxDocument): void {
    ...
    // 分割内容为多个 Section
    const sections = this.splitBySection(
      body.children || [],
      defaultSectionProps,
    );
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      // 渲染本页脚注
      const pageFootnoteIds = this.currentFootnoteIds.slice(
        pageFootnoteStartIndex,
      );
   ...
   * 按 Section 分割内容
   */
  private splitBySection(
    elements: IOpenXmlElement[],
    defaultProps?: ISectionProperties,
  ): ISection[] {
        ...
        const checkForPageBreak = (children: IOpenXmlElement[]) => {
          for (const child of children) {
            if (child.type === DomType.Break) {
              ...
              if (
                br.breakType === "page" ||
                br.breakType === "lastRenderedPageBreak"
              ) {
                hasPageBreak = true;
                return;
              }
   ...
   */
  private isPageBreakSection(
    prev: ISectionProperties | null,
    next: ISectionProperties | null,
  ): boolean {
    ...
    return (
      prevSize.orientation !== nextSize.orientation ||
  ...
  private createPageElement(
    sectionProps?: ISectionProperties | null,
  ): HTMLElement {
  ...
  private renderHeaderFooter(
    ...
    // 获取页眉/页脚内容
    const map =
      position === "header" ? this.document?.headers : this.document?.footers;
  ...
  private renderAllCommentBubbles(): void {
    ...
    const commentsLayer = this.createElement(
      "div",
  ...
  private positionCommentBubbles(): void {
    ...
    const dialogWidth = 1198;
    const pageWidth = pageRect.width;
    const bubbleWidth = 240;
    const bubbleMargin = 15;
    const bubbleRight = Math.max(
      (dialogWidth - pageWidth) / 2 - bubbleMargin - bubbleWidth,
      40,
    );

    console.log("nerd", pageRect);
    ...
    for (const [, range] of this.commentRanges) {
      ...
      if (isVisible) {
        ...
        range.panelElement.style.right = `${bubbleRight}px`;
  ...
  private drawCommentLine(commentId: string, active: boolean): void {
    ...
    if (
      !range ||
      !range.highlightElements.length ||
      !range.panelElement ||
      !this.svgLayer
    )
      return;
  ...
  private renderParagraph(element: IParagraphElement): HTMLElement {
    ...
    if (
      ...
      (p.childNodes.length === 1 && numberingContent)
    ) {
  ...
  private renderNumbering(props?: IParagraphProperties): HTMLElement | null {
    ...
    // 添加后缀（tab、space 等）
    const suffix = this.createElement(
      "span",
  ...
  private createCommentBubble(comment: ICommentElement): HTMLElement {
    ...
    bubble.addEventListener("mouseleave", () =>
      this.unhighlightComment(comment.id),
    );
  ...
  private highlightComment(commentId: string): void {
    ...
    if (range) {
      ...
      range.panelElement?.classList.add(
  ...
  private unhighlightComment(commentId: string): void {
    ...
    if (range) {
      ...
      range.panelElement?.classList.remove(
        `${this.classPrefix}-comment-bubble--active`,
      );
    }
  ...
  private editComment(comment: ICommentElement): void {
    ...
    if (newText !== null && newText !== currentText) {
      ...
      const paraEl: IParagraphElement = {
      ...
      if (range?.panelElement) {
        const contentEl = range.panelElement.querySelector(
  ...
  private applyParagraphStyles(
    ...
    if (props.indentation) {
      if (props.indentation.left)
        styles.push(`padding-left: ${props.indentation.left}`);
      if (props.indentation.right)
        styles.push(`padding-right: ${props.indentation.right}`);
      if (props.indentation.firstLine)
        styles.push(`text-indent: ${props.indentation.firstLine}`);
    }

    if (props.spacing) {
      if (props.spacing.before)
        styles.push(`margin-top: ${props.spacing.before}`);
      if (props.spacing.after)
        styles.push(`margin-bottom: ${props.spacing.after}`);
  ...
  private renderRun(element: IRunElement): HTMLElement | null {
    ...
    if (this.skipFieldContent) {
      const hasFieldEnd = (element.children || []).some(
        (child) =>
          child.type === DomType.ComplexField &&
  ...
  private applyRunStyles(element: HTMLElement, props: IRunProperties): void {
    ...
    if (props.highlight) {
      ...
      styles.push(
        `background-color: ${colorMap[props.highlight] || props.highlight}`,
      );
    }
  ...
  private renderBreak(element: IBreakElement): HTMLElement {
    switch (element.breakType) {
      ...
      case "lastRenderedPageBreak":
        ...
        return div;
      case "column":
        const colBreak = this.createElement(
          "span",
   ...
   * 渲染复杂域字符
   */
  private renderComplexField(
    element: IComplexFieldElement,
  ): HTMLElement | null {
    switch (element.charType) {
      ...
      case "separate":
        ...
        const fieldValue = this.evaluateFieldInstruction(
   ...
   * 渲染域指令
   */
  private renderFieldInstruction(
    element: IFieldInstructionElement,
  ): HTMLElement | null {
  ...
  private renderFootnoteReference(element: IFootnoteReference): HTMLElement {
    ...
    sup.addEventListener("click", () => {
      const footnoteEl = document.getElementById(
        `${this.classPrefix}-footnote-${element.id}`,
      );
  ...
  private renderEndnoteReference(element: IEndnoteReference): HTMLElement {
    ...
    // 点击跳转到尾注
    sup.addEventListener("click", () => {
      const endnoteEl = document.getElementById(
        `${this.classPrefix}-endnote-${element.id}`,
      );
   ...
   * 渲染页面脚注区域
   */
  private renderPageFootnotes(
    footnoteIds: string[],
    container: HTMLElement,
  ): void {
    if (footnoteIds.length === 0 || !this.document?.footnotes) return;

    const footnotesSection = this.createElement(
      "div",
      `${this.classPrefix}-footnotes-section`,
    );

    // 分隔线
    const separator = this.createElement(
      "hr",
  ...
  private renderDocumentEndnotes(wrapper: HTMLElement): void {
    ...
    const endnotesSection = this.createElement(
      "div",
